...
public class ResourceRefProtocolDispatcher implements DispatcherExtension {
    ...
    public String handle(Resource resource, DispatchInfo dispatchInfo) {
        ...
        String resourceRef = Optional.ofNullable(data.get("resourceRef")).map(String::valueOf).orElse("");
        String resourceRefType = Optional.ofNullable(data.get("resourceRefType")).map(String::valueOf).orElse("");
        String resourceRefName = Optional.ofNullable(data.get("resourceRefName")).map(String::valueOf).orElse("");
