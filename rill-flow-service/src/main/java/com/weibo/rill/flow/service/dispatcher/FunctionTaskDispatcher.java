    ...
    public String dispatch(DispatchInfo dispatchInfo) {
        try {
            ...
            if (functionTask.getResource() != null) {
                ...
                return descriptorManager.calculateResourceName(dispatchInfo.getUid(), input, dispatchInfo.getExecutionId(), functionTask.getResource().getConfigKey());
            }
    ...
    private String resourceNameProcess(DispatchInfo dispatchInfo, FunctionTask functionTask) {
        ...
        if (resource.isAbResource()) {
            Long uid = Optional.ofNullable(input.get("uid"))
                    ...
                    .orElse(0L);
            String calculatedResourceName = descriptorManager.calculateResourceName(uid, input, executionId, resource.getResourceName());
